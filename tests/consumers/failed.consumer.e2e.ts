...
describe('FailedConsumer', () => {
    ...
    describe('when consuming multiple failed events', () => {
        it('should process the action with right associated handlers', async () => {
            ...
            await createGroup(stream1, config.group)
            const eventsStream1 = await createFailedEvents({ group, clientId, stream: stream1, actions: [stream1Action1, stream1Action2] })
            const stream1Handler1 = jest.fn(async (_: Event, done: Done) => await done());
            const stream1Handler2 = jest.fn(async (_: Event, done: Done) => await done());
            ...
            await createGroup(stream2, config.group)
            const eventsStream2 = await createFailedEvents({ group, clientId, stream: stream2, actions: [stream2Action1, stream2Action2], })
            const stream2Handler1 = jest.fn(async (_: Event, done: Done) => await done());
            const stream2Handler2 = jest.fn(async (_: Event, done: Done) => await done());
            ...
            for (const call of stream1Handler1.mock.calls) {
                const [event, done] = call as [Event<{ id: string }, { id: string }>, Function]
